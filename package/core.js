class Slider {
    constructor(slider, options = {}) {
        ...
        this.prevButton = document.querySelector(`#${slider} .slider__arrow.slider__arrow--left`);
        this.nextButton = document.querySelector(`#${slider} .slider__arrow.slider__arrow--right`);
    ...
    startAutoSlide(callback, duration) {
    ...
    checkHoverForAutoSlide() {
        Array.from([this.bulletWrapper, this.nextButton, this.prevButton]).forEach((item) => {
            item &&
    ...
    prevSlide() {
        ...
        if (this.sliderOptions.verticalSlider) {
            this.container.style.transform = `translateY(-${this.slider.clientHeight * this.counter}px )`;
        }
        else {
            this.container.style.transform = `translateX(-${this.slider.clientWidth * this.counter}px )`;
        }
        this.addActiveSlideClass();
        this.bulletWrapper && this.setActiveBullet();
        !this.sliderOptions.infiniteSlide && this.checkArrowVisibility();
    ...
    nextSlide() {
        this.counter++;
        if (!this.sliderOptions.infiniteSlide &&
            this.counter > this.container.children.length - 1) {
            this.counter = this.container.children.length - 1;
        }
    ...
    clickBullet() {
        const bullets = document.querySelectorAll(`#${this.sliderId} .slider__bullet`);
        Array.from(bullets).forEach((item, index) => {
            item.addEventListener("click", () => {
                this.counter = index;
                this.container.style.transform = `translateX(-${this.container.clientWidth * this.counter}px )`;
                this.bulletWrapper && this.setActiveBullet();
            });
        });
    }
    ...
    setActiveBullet() {
        ...
        this.addActiveSlideClass();
        this.bulletWrapper.children[this.counter].classList.add("slider__bullet--active");
        !this.sliderOptions.infiniteSlide && this.checkArrowVisibility();
    }
    clearActiveBulletClass() {
        Array.from(this.bulletWrapper.children).forEach((item) => {
            item.classList.remove("slider__bullet--active");
        });
    }
    setBulletHover() {
        const sliderBullets = document.querySelectorAll(`#${this.sliderId} .slider__bullet`);
    ...
    setBulletImage() {
        ...
        Array.from(sliderBullets).forEach((item, index) => {
            (item.style.backgroundImage = `url("${this.sliderOptions.bulletImages &&
    ...
    setBulletDefaultImage() {
        const sliderBullets = document.querySelectorAll(`#${this.sliderId} .slider__bullet`);
        Array.from(sliderBullets).forEach((item, index) => {
            (item.style.backgroundImage = `url("${this.container.children[index].firstElementChild &&
